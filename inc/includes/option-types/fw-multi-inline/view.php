...
<div <?php echo fw_attr_to_html($div_attr) ?>>
	<div class="fw-multi-inline-group">
		<?php foreach ($option['value'] as $key => $options_group){
				...
				//icon
                if($type === 'icon'){
                    $html .= '<div class="fw-multi-inline-holder fw-multi-holding-icon">';
                    $html .= '<span class="fw-multi-inline-title">'.$option['fw_multi_options'][$key]['title'].'</span>';
                    $html .=  fw()->backend->option_type( 'icon' )->render(
                        $key,
                        array(
                            'type'  => 'icon',
                            'value' => fw_akg($key, $option['value']),
                ...
                //socicon
                if($type === 'socicon'){
                    $html .= '<div class="fw-multi-inline-holder fw-multi-holding-socicon">';
                    $html .= '<span class="fw-multi-inline-title">'.$option['fw_multi_options'][$key]['title'].'</span>';
                    $html .=  fw()->backend->option_type( 'icon' )->render(
                        $key,
                        array(
                            'type'  => 'icon',
                            'value' => fw_akg($key, $option['value']),
                            'set'   => $option['fw_multi_options'][$key]['set'],
                            'attr'  => array(
                                'data-fwmultioptions' => $group_name
                            )
                        ),
